...
class Process():
    ...
    def tuile_dispo(self):
        ...
        if _.Tuile.Color.rouge in lst:
            if len(lst) is not 0:
                ret = lst.remove(_.Tuile.Color.rouge)
    ...
    def position_dispo(self):
        ...
        for v in self.world.get_all_tuiles().values():
            if v.position is None or v.position not in lst:
                continue
        ...
        dct = sorted(dct, key=dct.get, reverse=False)
    ...
    def activer_pouvoir(self):
        ...
        response: 0 or 1
        """
        not_use = [_.Tuile.Color.noir, _.Tuile.Color.gris]
        use = [_.Tuile.Color.blanc]
    ...
    def process_question(self, q: _.Question):
        ...
        for v in self.world.get_all_tuiles().values():
            ...
            if s not in dct:
                dct[s] = []
            dct[s].append('{!s}-{!s}'.format(v.color, v.status))
        for k in sorted(dct, reverse=False):
            log.info('{} {!s:<4}: {!s}'.format(space_begin, k, dct[k]))
        log.info('{} {!s}'.format(space_begin, str(self.q)))
